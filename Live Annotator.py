...
Program name: Live Annotator
Programmed by: Shamoun Gergi
Recently updated: 18-08-2021
...
class Window(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(" Live Annotator")
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key_C:
            if self.mediaPlayer.duration() != 0:
                ...
                # If no video is open, but a csv is open, I set a limit of the fast forward.
                if self.xValues != []:
                    if self.mediaPlayer.position() + 50 < max(self.xValues):
                        self.set_position(self.mediaPlayer.position() + 50)
        ...
        if e.key() == Qt.Key_D:
            if self.mediaPlayer.duration() != 0:
                ...
                # If no video is open, but a csv is open, I set a limit of the fast forward.
                if self.xValues != []:
                    if self.mediaPlayer.position() + 200 < max(self.xValues):
                        self.set_position(self.mediaPlayer.position() + 200)
                    elif max(self.xValues) - self.mediaPlayer.position() <= 200:
                        self.set_position(max(self.xValues))
        ...
        if e.key() == Qt.Key_E:
            if self.mediaPlayer.duration() != 0:
                ...
                # If no video is open, but a csv is open, I set a limit of the fast forward.
                if self.xValues != []:
                    if self.mediaPlayer.position() + 5000 < max(self.xValues):
                        self.set_position(self.mediaPlayer.position() + 5000)
                    elif max(self.xValues) - self.mediaPlayer.position() <= 5000:
                        self.set_position(max(self.xValues))
    ...
    def wheelEvent(self,event):
        ...
        for step in range(1, abs(steps) + 1):
            if self.mediaPlayer.duration() != 0:
                # If there a video is open.
                self.set_position(self.mediaPlayer.position() + 200*vector)
            else:
                # If no video is open, but a csv is open, I set a limit of the fast forward.
                if self.xValues != []:
                    if self.mediaPlayer.position() + 200*vector < max(self.xValues):
                        self.set_position(self.mediaPlayer.position() + 200*vector)
                    elif max(self.xValues) - self.mediaPlayer.position() <= 200:
                        self.set_position(max(self.xValues))
    ...
    def open_annotation(self):
        ...
        if self.annotationFile[-3:] == "csv":
            ...
            if os.path.isfile(self.annotationFile.replace("csv", "mp4")):
                # If a mp4 video file exists.
                ...
                message = QMessageBox()
                message.setWindowTitle("Fail")
                message.setText("An error is caused by one of the following reasons: \n\n"
                "\t - There is no mp4-file in the same directory as the csv-file.\n"
                "\t - The video file and the csv-file have different names.\n\n"
                "The data from the csv-file will be plotted anyway. ")
                x = message.exec_()  # this will show our messagebox 


            
            
            # Data from the csv-file is moved to the lists.
            with open(self.annotationFile, newline='') as csvfile:
                ...
                for row in spamreader:
                    ...
                    try:
                        ...
                        #  If the element is a string (the column titles) we continue to the next element/ csv-raw.
                        continue
    ...
    def play_video(self):
        ...
        elif self.videoOpened:
            # If the state is pause/ stop, but the video is opened.
    ...
    def position_changed(self, position):
        ...
        # Below, some mathematical formulas are used to caluclate the video position in the format mm:ss. This will be shown on the right of the video slider.

        if position >= self.mediaPlayer.duration():
            position = self.mediaPlayer.duration()

        if (position//1000)//60 < 10:
            position_left_digits = "0" + str((position//1000)//60)
        else:
            position_left_digits = str((position//1000)//60)

        if (position//1000)%60 < 10:
            position_right_digits = "0" + str((position//1000)%60)
        else:
            position_right_digits = str((position//1000)%60)

        # Updates the duration of the video.
        self.durationLabel.setText(position_left_digits + ":"+ position_right_digits)

    def duration_changed(self, duration):
        self.slider.setRange(0, duration)

        # Below, some mathematical formulas are used to caluclate the video duaration in the format mm:ss. This will be shown on the right of the video slider.

        if (duration//1000)//60 < 10:
            duration_left_digits = "0" + str((duration//1000)//60)
        else:
            duration_left_digits = str((duration//1000)//60)

        if (duration//1000)%60 < 10:
            duration_right_digits = "0" + str((duration//1000)%60)
        else:
            duration_right_digits = str((duration//1000)%60)

        # Updates the maximal length of the video.
        self.lengthLabel.setText("/ "+ duration_left_digits + ":"+ duration_right_digits)
